...
import {
  ...
  submitClientForm, sendWameMessage,
...
function ClientDetail({
  ...
  async function sendLinkViaWame() {
    const wa = client.whatsapp?.replace(/\D/g, "") ?? ""
    if (!wa || !fillUrl) return
    setWameSending(true)
    setWameStatus("idle")
    const msg = `Olá ${client.name}! Preparei um formulário de cadastro para você. Por favor, preencha seus dados acessando o link a seguir: ${fillUrl}`
    const result = await sendWameMessage(wa, msg)
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="shrink-0 border-t border-border p-4 space-y-3">
        ...
        {/* Botão enviar link via WhatsApp */}
        {fillUrl && client.whatsapp && (
          <button
            onClick={sendLinkViaWame}
            disabled={wameSending}
          ...
          >
            {wameSending ? (
              <><span className="size-3.5 border-2 border-current border-t-transparent rounded-full animate-spin" />Enviando...</>
            ) : (
              <>
        ...
        {wameStatus === "ok" && (
          <div className="flex items-center gap-2 text-xs text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 rounded-lg px-3 py-2">
            <Check className="size-3.5" />Link enviado com sucesso via WhatsApp!
          </div>
        )}
        {wameStatus === "error" && (
          <div className="text-xs text-amber-400 bg-amber-400/10 border border-amber-400/20 rounded-lg px-3 py-2">
            <p className="font-semibold">Erro no wame — abrindo wa.me</p>
